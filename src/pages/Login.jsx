...
return (
        <div className="w-full min-h-[calc(100vh-155px)] flex justify-center md:justify-end items-center p-6 md:pr-32 relative"
             style={{
                 ...
                 backgroundPosition: 'center center' // Чітко фіксуємо центр зображення
             }}>

            <div className="bg-white rounded-[2rem] p-10 w-full max-w-md shadow-2xl relative z-10 font-['Inter']">

                {/* Заголовок шрифтом El Messiri */}
                <h1 className="text-3xl font-['El_Messiri'] font-bold mb-8 text-[#1A1A1A] text-center md:text-left">Увійти</h1>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="password" name="password" value={formData.password} onChange={handleChange} required placeholder="Введіть пароль" className="w-full px-5 py-3.5 rounded-full border border-gray-300 focus:outline-none focus:border-[#42705D] transition text-sm text-gray-700" />

                        {/* Посилання "Забули пароль?" */}
                        <div className="text-right mt-2 pr-4">
                            <Link to="/reset-password" className="text-[13px] text-blue-600 hover:text-[#42705D] transition font-medium">Забули пароль?</Link>
                        </div>
                    </div>
                    <button type="submit" className="w-full bg-[#1A1A1A] text-white font-medium rounded-full py-3.5 hover:bg-gray-800 transition mt-2">Увійти</button>
                </form>
                ...
                <div className="mt-8 text-center text-[13px] text-gray-600">
                    У вас не має акаунту? <Link to="/register" className="text-blue-600 hover:text-[#42705D] hover:underline font-semibold ml-1 transition">Зареєструватись</Link>
                </div>
            </div>
        </div>
    );
